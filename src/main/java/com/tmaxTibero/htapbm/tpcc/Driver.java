...
//    public PrintWriter freport_file;
    ...
    public int runTransaction(final int t_num, final int numWare, final int numConn) {
        ...
        while (!TpccThread.terminateCheck[t_num]) {
            try {
        ...
        pStmts.closeConnection();
        logger.info("Session" + t_num + " terminated after {} transactions", count);
        TpccThread.terminateCheck[t_num] = false;
        Tpcc.currentSessionCount.decrementAndGet();
